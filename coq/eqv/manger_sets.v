...
(* Require Import CAS.coq.theory.set. ALL of this moved to coq.eqv.set *) 
...
TGG22: 
The first is is easy to derive, the other two are false (we roll our own equalities)! 
...
(* this already exists in list_congruences   
Lemma fold_right_congruence_for_manger : 
...
Lemma addP_gen_idempotent : ∀ x y : P, eqP x y = true → eqP (addP x y) y = true.
Proof. intros x y B.
...
(* name change 
   fold_left_congruence -> fold_left_congruence_for_manger 
*) 
Lemma fold_left_congruence_for_manger : 
  forall (X Y : list (A * P)) 
  (pa : A) (pb : P),  (forall u v, (u, v) [in] X -> eqA u pa = true) ->
  (forall u v, (u, v) [in] Y -> eqA u pa = true) ->
  X =S= Y ->
  [fold_left (λ '(s1, t1) '(_, t2), (s1, addP t1 t2)) X (pa, pb)]
  =S= 
  [fold_left (λ '(s1, t1) '(_, t2), (s1, addP t1 t2)) Y (pa, pb)].
Proof. intros X Y pa bp pa_unique_in_X pa_unique_in_Y eqXY.
       apply set_equal_with_cons_left. 
       apply (fold_left_congruence _ _ (brel_product eqA eqP) (brel_product eqA eqP)).
       - intros [a1 p1] [a2 p2] [a3 p3] [a4 p4] I J.
         apply brel_product_elim in I, J.
       ...
       - admit.
         (* eqXY : X =S= Y
            ============================  
            brel_list (brel_product eqA eqP) X Y = true *) 
Admitted.          
(*
Proof.
  intros ? ? ? ? Ha Hb Hc.
  ...
  + intros (au, pu) Hd.
    ...
    - apply brel_product_elim in Hd.
      ...
      * eapply trnP.    
        -- exact Hdr.
        -- (*
          eqP (fold_left (λ t1 t2 : P, addP t1 t2) (map snd X) pb)
              (fold_left (λ t1 t2 : P, addP t1 t2) (map snd Y) pb) = true *) 
          repeat rewrite fold_symmetric;  (* use fold_symmetric_with_equality! *) 
             try assumption.
           ++ eapply fold_right_congruence_for_manger.
              ** intros ? ? ?.
                 eapply symP.
                 eapply addP_assoc.
              ** eapply addP_com.
              ...
              ** apply map_preservs_equivalence_on_second.
                 ...
                 --- eapply refAP.
                 --- split; assumption.
           ++ admit. (* ∀ x y z : P, addP x (addP y z) = addP (addP x y) z *)
           ++ admit. (* ∀ y : P, addP pb y = addP y pb *) 
           ++ admit. (* ∀ x y z : P, addP x (addP y z) = addP (addP x y) z *) 
           ++ admit. (* ∀ y : P, addP pb y = addP y pb *)
    - inversion Hd.
  + 
    ...
    - apply brel_product_elim in Hd.
      ...
      * eapply trnP.
        ...
        -- eapply fold_right_congruence_for_manger.
           ** intros ? ? ?; eapply symP;eapply addP_assoc.
           ** exact addP_com.
           ** intros ? ? ? ? Hxy Hwv;
                eapply cong_addP;
                try assumption.
...
Proof.
  ...
  as Yf.
  eapply fold_left_congruence_for_manger.
  + intros ? ? Hd.
    rewrite HeqXf in Hd.
...
Proof.
  ...
  eapply trnP with 
    (addP 
      (big_plus zeroP addP snd (List.filter (λ '(x, _), eqA a x) V))
      (big_plus zeroP addP snd 
      (List.filter (λ '(x, _), eqA a x)
       [fold_left (λ '(s1, t1) '(_, t2), (s1, addP t1 t2))
         (filter (λ '(s2, _), eqA au s2) V) (au, av)]))).
...
Proof.
  ...
  eapply trnP with 
    (addP (big_plus zeroP addP snd (List.filter (λ '(x, _), eqA a x) Ub))
     (big_plus zeroP addP snd
        (List.filter (λ '(x, _), eqA a x) V))).
  eapply cong_addP.
  eapply refP.
  eapply mmsn_invariant_big_plus;
  exact Ha.
  eapply symP.
  apply big_plus_distributes_over_concat;
  try assumption.
  split;
  [eapply zeropLid | eapply zeropRid].
  rewrite filter_app.
...
Proof.
  intros ? ? ? ? ? Ha Hb.
  rewrite fold_symmetric in Ha, Hb;  (* use fold_symmetric_with_equality! *) 
  try assumption.
  assert (Hc : 
...
Proof.
  ...
  +
    ...
    rewrite fold_symmetric.  (* use fold_symmetric_with_equality! *) 
    rewrite <-list_filter_lib_filter_same in Hb.
    ...
    exact Ha.
    admit. (* eapply addP_assoc_cong.*) 
    admit. (*eapply addP_com_cong. *) 
  +
    simpl in * |- *.
    (* case analysis *)
    case_eq (eqA a ax);
...
Lemma reorder_addP (a b c : P) : 
  eqP (addP a (addP b c)) (addP b (addP a c)) = true.
Proof. assert (B := addP_assoc a b c). apply symP in B. 
       assert (C := addP_assoc b a c).
       assert (D : eqP (addP (addP a b) c) (addP (addP b a) c) = true).
       {
         exact (cong_addP _ _ _ _ (addP_com a b) (refP c)).
       }
       exact (trnP _ _ _ (trnP _ _ _ B D) C). 
Qed. 
  ...
  +
    ...
    - eapply symP.
      ...
      * (* OUCH!
           ...
           eapply refP. *)
        apply symP. apply addP_assoc.
    - (* OUCH! 
         rewrite addP_com_cong, <-addP_assoc_cong.
         eapply cong_addP.
         eapply refP.
         rewrite addP_com_cong;
         eapply refP. 
...
Proof.
  ...
  -
    ...
    + assert (Hd : no_dup eqA (map fst ([MMSN] Y (ax, bx))) = true).
       ...
       unfold big_plus in He |- *; cbn. (* TGG22 : WHY UNFOLD? *) 
       repeat rewrite <-list_filter_lib_filter_same, filter_app, map_app in He.
       cbn in He.
       rewrite <-list_filter_lib_filter_same, filter_app in He.
       rewrite filter_argument_swap, filter_empty, app_nil_l, fold_left_simp in He.
      * cbn in He; rewrite Hc in He.
        cbn in He.
        rewrite <-list_filter_lib_filter_same, filter_app, map_app.
        ...
        (* use fold_symmetric_with_equality! *)
        
        -- rewrite fold_symmetric in HeqV; try assumption.
           ++ subst.
              (* now replace in He fold_right_right *)
              assert (Hg : 
                       eqP p
                         (addP
                            (fold_right addP zeroP
    ...
    + 
      ...
      we are home 
      *)
      rewrite in_set_equality_false in Hb; try assumption.
      * eapply IHx; try assumption.
      * case_eq (eqA a ax); intros Hd.
        -- rewrite (symA _ _ Hd) in Hc; congruence.
        -- reflexivity.
Admitted. 
