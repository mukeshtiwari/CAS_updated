...
Section Theory.
  ...
  Lemma A_bs_or_and_left_distributive  :
     A_bs_left_distributive brel_eq_bool bop_or bop_and. 
  Proof. intros x y z.
         destruct x; destruct y; destruct z; compute; reflexivity.
  Qed. 

Lemma A_bs_or_and_right_distributive  : 
     A_bs_right_distributive brel_eq_bool bop_or bop_and.
Proof. intros x y z.
       destruct x; destruct y; destruct z; compute; reflexivity.
Qed. 
...
Lemma A_bs_or_and_right_absorptive  : 
     A_bs_right_absorptive brel_eq_bool bop_or bop_and.
Proof. intros x y. destruct x; destruct y; compute; reflexivity. Qed. 

(* strict absorption 
Lemma A_bs_or_and_not_left_strictly_absorptive  : 
     A_bs_not_left_strictly_absorptive bool brel_eq_bool A_bs_and A_bs_or.
Proof. exists (true, true). compute. right. auto. Defined. 

Lemma A_bs_or_and_not_right_strictly_absorptive  : 
     A_bs_not_right_strictly_absorptive bool brel_eq_bool A_bs_and A_bs_or.
Proof. exists (true, true). compute. right. auto. Defined.
 

Lemma A_bs_or_and_right_left_absorptive  : 
     A_bs_right_left_absorptive bool brel_eq_bool A_bs_and A_bs_or.
Proof. intros x y. destruct x; destruct y; compute; reflexivity. Qed. 

Lemma A_bs_or_and_right_right_absorptive  : 
...
Definition A_bs_or_and_properties : A_bs_properties brel_eq_bool bop_or bop_and := 
  {| 
     A_bs_left_distributive_d  := inl A_bs_or_and_left_distributive
   ; A_bs_right_distributive_d := inl A_bs_or_and_right_distributive
   ; A_bs_left_absorptive_d    := inl A_bs_or_and_left_absorptive
   ; A_bs_right_absorptive_d   := inl A_bs_or_and_right_absorptive
  |}.
...
; A_bs_id_ann_props :=
    {|
      A_id_ann_plus_times_d := A_Id_Ann_Equal _ _ _ A_bs_or_and_exists_id_ann_equal
    ; A_id_ann_times_plus_d := A_Id_Ann_Equal _ _ _ A_bs_and_or_exists_id_ann_equal
    |}
; A_bs_props        := A_bs_or_and_properties
; A_bs_ast          := Ast_and_or
|}.
...
Definition A_mcas_bs_or_and := A_MCAS_bs (A_classify_bs A_bs_or_and).   
  
End AMCAS.   

Section CAS.

Definition bs_or_and_properties : @bs_properties bool := 
  {| 
...
; bs_id_ann_props :=
    {|
      id_ann_plus_times_d := Id_Ann_Equal false 
    ; id_ann_times_plus_d := Id_Ann_Equal true 
    |}
; bs_props        := bs_or_and_properties
; bs_ast          := Ast_and_or
|}.
...
Definition mcas_bs_or_and := MCAS_bs (classify_bs bs_or_and).   
...
Theorem correct_bs_or_and : bs_or_and = A2C_bs (A_bs_or_and). 
Proof. compute. reflexivity. Qed. 

Theorem correct_mcas_bs_and_or : mcas_bs_or_and = A2C_bs_mcas A_mcas_bs_or_and.
Proof. compute. reflexivity. Qed. 
